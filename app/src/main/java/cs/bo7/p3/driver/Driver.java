...
/**
 * A Driver used for auto testing the project backend.
 *
 * @author anya
...
public class Driver {
  ...
  /**
   * Uploads client information to the application from the file at the given path. Then
   * de-serializes all the other clients.
   *
   * @param path the path to an input csv file of client information with lines in the format:
   *     LastName,FirstNames,Email,Address,CreditCardNumber,ExpiryDate (the ExpiryDate is stored in
   *     the format YYYY-MM-DD)
   */
  public static void uploadClientInfo(String path) {
    try {
      AccountManager.uploadClientInfo(path);
   ...
   *     the dates are in the format YYYY-MM-DD; the price has exactly two decimal places)
   */
  public static void uploadFlightInfo(String path) {
    FlightManager manager = new FlightManager();
    try {
      manager.uploadFlightInfo(path);
    } catch (FileNotFoundException | ParseException e) {
      e.printStackTrace();
    }
  }
  ...
  public static String getClient(String email) {
    String returnValue = "";
    try {
      returnValue = AccountManager.findClient(email).toString();
    } catch (Exception e) {
      e.printStackTrace();
    }
    return returnValue;
   ...
   *     ArrivalDateTime,Airline,Origin,Destination,Price (the dates are in the format YYYY-MM-DD;
   *     the price has exactly two decimal places)
   */
  public static String getFlights(String date, String origin, String destination) {
    FlightManager manager = new FlightManager();
    return manager.getFlights(date, origin, destination);
  }
   ...
   *     flight, in the format: Number,DepartureDateTime,ArrivalDateTime,Airline,Origin,Destination
   *     followed by total price (on its own line, exactly two decimal places), followed by total
   *     duration (on its own line, in format HH:MM).
   */
  public static String getItineraries(String date, String origin, String destination) {
    ...
    manager.search(date, origin, destination);
   ...
   *     price (on its own line, exactly two decimal places), followed by total duration
   ...
   *     depart from origin and arrive at destination on the given date with stopovers at
   *     or under 6 hours. Each itinerary in the output should contain one line per flight,
   *     in the format: Number,DepartureDateTime,ArrivalDateTime,Airline,Origin,Destination
   *     followed by total price (on its own line, exactly two decimal places), followed by total
   *     duration (on its own line, informat HH:MM).
   */
